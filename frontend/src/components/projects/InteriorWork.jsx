...
const InteriorWork = ({ interiorItems, setInteriorItems, customPricing }) => {
  ...
  // useEffect(() => { calculateTotalCost(); }, [interiorItems, pricing]);
  ...
  //   try {
  //     if (type === 'doors') {
  //       return pricing.interior.doors[condition]?.price || 0;
  ...
  const getPriceDisplay = (type, item) => {
    if (!pricing?.interior) return '£0.00';
    ...
    <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
      ...
      {pricing?.interior && !pricingLoading && (
        <div className="bg-blue-50 border border-blue-200 rounded-lg p-4 mb-6">
          <h4 className="text-sm font-medium text-blue-900 mb-2">Current Interior Pricing</h4>
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-2 text-xs">
            <div>
              <span className="font-medium text-blue-800">Doors:</span>
              <div className="text-blue-600">
                Easy Prep: £{pricing.interior.doors?.easy_prep?.price || 0}<br />
                Medium Prep: £{pricing.interior.doors?.medium_prep?.price || 0}<br />
                Heavy Prep: £{pricing.interior.doors?.heavy_prep?.price || 0}
              </div>
            </div>
            <div>
              <span className="font-medium text-blue-800">Fixed Windows:</span>
              <div className="text-blue-600">
                Small: £{pricing.interior.fixedWindows?.small?.price || 0}<br />
                Medium: £{pricing.interior.fixedWindows?.medium?.price || 0}<br />
                Large: £{pricing.interior.fixedWindows?.big?.price || 0}
              </div>
            </div>
            <div>
              <span className="font-medium text-blue-800">Other Items:</span>
              <div className="text-blue-600">
                Stairs: £{pricing.interior.stairs?.price || 0}<br />
                Radiators: £{pricing.interior.radiators?.price || 0}<br />
                Skirting: £{pricing.interior.skirtingBoards?.price || 0}
              </div>
            </div>
          ...
          <div key={type} className="mb-6">
            <div className="flex items-center justify-between mb-3">
              ...
              <div className="text-sm text-gray-500">
                Items: {interiorItems[type].length}
              </div>
            </div>
